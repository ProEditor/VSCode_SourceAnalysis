...
var _amdLoaderGlobal = this,
    define;
console.log("start execute loader")
var AMDLoader;
(function(AMDLoader) {
    // ------------------------------------------------------------------------
    // Utilities
    ...
    var Utilities = (function() {
        ...
        Utilities.isAbsolutePath = function(url) {
            return (Utilities.startsWith(url, 'http://') || Utilities.startsWith(url, 'https://') || Utilities.startsWith(url, 'file://') || Utilities.startsWith(url, '/'));
        };
        ...
        ConfigurationOptionsUtil.mergeConfigurationOptions = function(overwrite, base) {
            ...
            Utilities.forEachProperty(overwrite, function(key, value) {
                if (key === 'bundles' && typeof result.bundles !== 'undefined') {
                    ...
                    Utilities.forEachProperty(value, function(key2, value2) {
    ...
    var ModuleManager = (function() {
        ...
        ModuleManager.prototype._onLoadError = function(id, err) {
            ...
            var seenModuleId = {},
                queueElement, someoneNotified = false,
                queue = [];
            queue.push(id);
        ...
        ModuleManager.prototype._resolveShimmedDependency = function(moduleId, dependencyId, loadCallback) {
            ...
            if (defineInfo.dependencies.length > 0) {
                this.defineModule(Utilities.generateAnonymousModule(), defineInfo.dependencies, function() {
                    return loadCallback(dependencyId); }, null, null, new ModuleIdResolver(this._config, dependencyId));
            } else {
    ...
    var OnlyOnceScriptLoader = (function() {
        ...
        OnlyOnceScriptLoader.prototype.load = function(scriptSrc, callback, errorback, recorder) {
            ...
            this.actualScriptLoader.load(scriptSrc, function() {
                return _this.triggerCallback(scriptSrc); }, function(err) {
                return _this.triggerErrorback(scriptSrc, err); }, recorder);
        };
        ...
        NodeScriptLoader.prototype.load = function(scriptSrc, callback, errorback, recorder) {
            ...
            if (/^node\|/.test(scriptSrc)) {
                ...
                this._fs.readFile(scriptSrc, { encoding: 'utf8' }, function(err, data) {
                    ...
                    var contents, prefix = '(function (require, define, __filename, __dirname) { ',
                        suffix = '\n});';
    ...
    var global = _amdLoaderGlobal,
        ...
        isWebWorker, isElectronRenderer, isElectronMain, isNode, scriptLoader, moduleManager, loaderAvailableTimestamp;
